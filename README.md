...
evidence** for the foreground session's latest primary request (even in tree
scope). Long identifiers wrap without truncation. Request model prefers the
outgoing JSON body — over a WebSocket session's outgoing frame, over the
configured model ID from the `model.request` hook as the transport-independent
fallback. Response model comes from the local
...
echoed instructions do not hide model evidence (maximum nesting 128, model ID
512 characters, request body 4 MiB). Mock HTTP-hook, WebSocket-observer, and
sidebar tests cover collection and retention.
