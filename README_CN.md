...
除了 `cli.json` 中的侧栏插件，还需在 `opencode.jsonc` 中加载同名服务端插件：
...
每份配置只保留一个 metrics 条目，保留其他插件。已安装副本不会自动跟随 npm 更新，
升级后需重新打开 TUI，已挂载的 CLI 插件不热更新。
本地开发时服务端和侧栏配置均指向本仓库 `src/` 目录的绝对路径，
分别发现服务端和 TUI 入口。构建不会修改现有配置。
回退时将 CLI 条目锁定为 `opencode-metrics@0.4.2`，并移除新增的服务端入口。

展开侧栏可见「Request model」「Response model」「Model evidence」：分别来自实际发出的
...
CLI 插件本身不能观察原始 HTTP 响应；模型证据需加载本页说明的服务端入口。
